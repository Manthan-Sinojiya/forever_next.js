...
export default function BlogListingPage() {
  ...
  React.useEffect(() => {
    fetch("/api/admin/blogs?published=true")
      .then((res) => res.json())
      .then((data) => {
      ...
      <main className="flex-1 w-full bg-slate-50 min-h-screen pt-16 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loading ? (
              <div className="text-center col-span-full py-10">Loading posts...</div>
            ) : blogPosts.length === 0 ? (
              <div className="text-center col-span-full py-10">No blog posts found.</div>
            ) : blogPosts.map((post: any) => (
              <div key={post._id || post.id} className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-md transition-shadow group flex flex-col">
                <div className="relative w-full h-56 overflow-hidden">
                  <Image src={post.image || "/placeholder.jpg"} alt={post.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex items-center gap-4 text-xs font-semibold text-slate-400 mb-4">
                    <div className="flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5" /> {post.publishedAt ? new Date(post.publishedAt).toLocaleDateString() : new Date(post.createdAt).toLocaleDateString()}
                    </div>
                    <div className="flex items-center gap-1">
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed mb-6 line-clamp-3 flex-1" dangerouslySetInnerHTML={{ __html: post.content.substring(0, 150) + "..." }} />
                  <Link href={`/blog/${post.slug || post._id}`} className="text-emerald-600 font-bold text-sm flex items-center gap-2 group/link w-max mt-auto">
