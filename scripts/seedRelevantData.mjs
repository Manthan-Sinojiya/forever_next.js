...
mongoose.connect(MONGODB_URI)
  .then(async () => {
    ...
    // 3. Seed Custom Pages
    try {
      const { CustomPage } = await import("./models/CustomPage.js");
    ...
    } catch (e) { console.error("Custom Pages seed failed: ", e.message); }

    // 4. Seed Testimonials
    try {
      const { Testimonial } = await import("./models/Testimonial.js");
    ...
    } catch (e) { console.error("Testimonials seed failed: ", e.message); }
    ...
    } catch (e) { console.error("FAQ seed failed: ", e.message); }
